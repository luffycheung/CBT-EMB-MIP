...
Project\]\([https://developer.android.com/studio/projects/add-native-code.html\#existing-project](https://developer.android.com/studio/projects/add-native-code.html#existing-project)\)
...
2. 从 `app/build/outputs/apk/ 路径中选择 APK，并点击OK。`
...
library。需要在编译文件中添加 \`cmake\_minimum\_required\(\) 和
...
当你使用 \`add\_library\(\)，将一个源文件（source file）或库添加到你的
...
include\_directories\(\) 命令，并指定头文件路径：\`
...
目录中自动添加源文件和头文件。你可以多次使用 \`add\_library\(\)
...
将 \`find\_library\(\) 命令添加到你的 CMake 构建脚本中，这样就可以定位 NDK
...
library）。你可以在 CMake 构建脚本中使用 \`add\_library\(\)

命令将源码编译进本地库。这时就需要提供你的本地 NDK

安装路径，通常将该路径保存在 ANDROID\_NDK 变量中，这样 Android Studio

可以自动为你识别。\`

下面的命令告诉 CMake 去构建

\`android\_native\_app\_glue.c，这个命令可以管理 NativeActivity
...
然后你需要使用 \`set\_target\_properties\(\)
...
CPU 架构。你可以使用 ANDROID\_ABI 路径变量，将多个 ABI
...
在 CMake 构建脚本中使用 \`target\_link\_libraries\(\)
...
\[1\]: [http://upload-images.jianshu.io/upload\_images/1132780-b929e473b22bba72.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240](http://upload-images.jianshu.io/upload_images/1132780-b929e473b22bba72.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
